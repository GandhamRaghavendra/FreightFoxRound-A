...
public class WeatherInfoServiceImpl implements WeatherInfoService {
	...
	public WeatherInfo saveWeatherInfo(RequestDto dto) throws JsonMappingException, JsonProcessingException, WeatherInfoException {
		...
		// ?q=19.9269,84.5389
		// &date=2020-10-15
		// &key=3d39530c9e9b4146a6c71400231904
		// &format=json)
		...
		if(readTree.path("data").path("weather") != null) {
			...
			String sunrise = readTree.path("data").path("weather").get(0).path("astronomy").get(0).path("sunrise").asText();
			
			String sunset = readTree.path("data").path("weather").get(0).path("astronomy").get(0).path("sunset").asText();
			
			String moonrise = readTree.path("data").path("weather").get(0).path("astronomy").get(0).path("moonrise").asText();
			
			String moonset = readTree.path("data").path("weather").get(0).path("astronomy").get(0).path("moonset").asText();
